<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-dropdown v-has-any-permission="['user:add','user:delete','user:reset','user:export']" trigger="click" class="filter-item">
        ...
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item v-has-permission="['user:add']" @click.native="add">{{ $t('table.user.add') }}</el-dropdown-item>
          <el-dropdown-item v-has-permission="['user:add']" @click.native="addWasteUser">{{ $t('waste.user.add') }}</el-dropdown-item>
    ...
    >
      ...
      <el-table-column :label="$t('waste.user.username')" prop="username" :show-overflow-tooltip="true" align="center" min-width="120px">
      ...
      <el-table-column :label="$t('waste.user.depotName')" :show-overflow-tooltip="true" align="center" min-width="150px">
        <template slot-scope="scope">
          <span>请点击编辑</span>
        </template>
        <!-- <template slot-scope="scope">
          <li v-for="item in scope.row.depotList" :key="item">
            {{ item }}
          </li>
        </template> -->
      </el-table-column>
      <el-table-column :label="$t('waste.user.enterpriseName')" align="center" min-width="100px">
        <template slot-scope="scope">
          <span>请点击编辑</span>
        </template>
        <!-- <template slot-scope="scope">
           <li v-for="item in scope.row.enterpriseInfo" :key="item.enterpriseName">
            {{ item.enterpriseName }}
          </li>
        </template> -->
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    edit(row) {
      ...
      //   row.roleId = roleId
      // }
      this.$get(`system/user/${row.uuid}`).then((r) => {
        row.roles = r.data.data
